...
async function getCount(modelName, nsecode, count) {
  // console.log(modelName, nsecode, count);
...
async function showPopup(company, tableName, sector, modelName) {
  ...
  try {
    ...
    // console.log(`freqData --> ${JSON.stringify(freqData)}`);
    ...
    const labels = freqData.map(item => item.date);
...
async function init() {
  await fetchDataWithDelay('Champions Intraday', 'intraday-table', 100, 'IntradayData', 'INTRADAY');
  await fetchDataWithDelay('Champions Over Brought', 'overbrought-table', 100, 'OverBroughtData', 'OVERBOUGHT');
  await fetchDataWithDelay('Champions Positional', 'positional-table', 100, 'PositionalData', 'POSITIONAL');
...
init();
setInterval(init, 20000); // Refresh every 60 seconds
